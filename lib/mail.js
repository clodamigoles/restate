...
async function getSiteSettingsForMail() {
  if (_settingsCache && Date.now() - _settingsCacheTime < 60_000) return _settingsCache;
  try {
    await dbConnect();
    const s = await Settings.findOne({ key: 'global' }).lean();
    _settingsCache = { siteName: s?.siteName || '', emailFrom: s?.emailFrom || '' };
    _settingsCacheTime = Date.now();
  } catch {
    _settingsCache = { siteName: '', emailFrom: '' };
  }
...
export async function sendEmail({ to, subject, html, text }) {
  try {
    ...
    const { siteName, emailFrom } = await getSiteSettingsForMail();
    const fromName = siteName || 'Maxo Destinations';
    const fromAddr = process.env.SMTP_FROM || emailFrom || 'support@maxodestinations.com';
